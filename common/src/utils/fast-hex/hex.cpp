...
inline static __m256i unhexBitManip(const __m256i value) {
  ...
  // (x >> 6) * 9 is x * 8 + x:
  // ((x >> 6) << 3) + (x >> 6)
  // We need & 0b11 to emulate 8-bit operations (narrowest shift is 16b) -- or a
  // left shift
  // (((x >> 6) & 0b11) << 3) + ((x >> 6) & 0b11)
  // or
  // tmp = (x >> 6) & 0b11
...
static const __m256i ROT2 =
    _mm256_setr_epi8(-1, 0, -1, 2, -1, 4, -1, 6, -1, 8, -1, 10, -1, 12, -1, 14,
...
void decodeHexBMI(uint8_t* __restrict__ dest, const uint8_t* __restrict__ src,
                  size_t len) {
...
void decodeHexVec(uint8_t* __restrict__ dest, const uint8_t* __restrict__ src,
  ...
  const __m256i A_MASK = _mm256_setr_epi8(
      0, -1, 2, -1, 4, -1, 6, -1, 8, -1, 10, -1, 12, -1, 14, -1, 0, -1, 2, -1,
      4, -1, 6, -1, 8, -1, 10, -1, 12, -1, 14, -1);
  const __m256i B_MASK = _mm256_setr_epi8(
      1, -1, 3, -1, 5, -1, 7, -1, 9, -1, 11, -1, 13, -1, 15, -1, 1, -1, 3, -1,
